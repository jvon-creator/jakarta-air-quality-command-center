...
.hero-grid { position: relative; z-index: 2; display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 24px; align-items: end; }
.hero-eyebrow {
...
.hero-title { font-family: 'Barlow Condensed', 'Inter', sans-serif; font-size: clamp(2.05rem, 3.1vw, 3.65rem); line-height: .92; letter-spacing: .012em; text-transform: uppercase; margin: 0; max-width: 980px; text-wrap: balance; color: var(--ink) !important; }
.hero-copy { margin-top: 12px; color: var(--slate) !important; max-width: 900px; font-size: .97rem; line-height: 1.72; font-weight: 500; }
.hero-meta { min-width: 300px; border: 1px solid var(--line); border-radius: 24px; background: rgba(255,255,255,.92); backdrop-filter: blur(18px); padding: 18px 19px; box-shadow: inset 0 1px 0 rgba(255,255,255,.90), var(--shadow-soft); }
...
.hero-meta .value { font-family: 'Barlow Condensed', 'Inter', sans-serif; font-size: 1.32rem; line-height: 1.08; color: var(--ink) !important; margin-top: 7px; }
.hero-meta .status { display: inline-flex; margin-top: 13px; padding: 8px 11px; border-radius: 999px; border: 1px solid rgba(0,166,118,.34); color: #064E3B !important; background: rgba(0,166,118,.13); font-weight: 800; font-size: .78rem; }
...
.pill-row { display: flex; flex-wrap: wrap; gap: 9px; margin: 12px 0 2px 0; }
.pill { display: inline-flex; align-items: center; padding: 8px 11px; border-radius: 999px; background: #FFFFFF; border: 1px solid var(--line); color: var(--slate) !important; font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: .72rem; font-weight: 800; box-shadow: 0 8px 18px rgba(15,23,42,.05); }
...
.decision-grid { position: relative; z-index: 2; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; margin-top: 16px; }
.decision-tile { border-radius: 18px; padding: 12px 13px; background: #FFFFFF; border: 1px solid var(--line); box-shadow: var(--shadow-soft); }
...
.decision-tile .tile-value { margin-top: 6px; font-family: 'Barlow Condensed', 'Inter', sans-serif; font-weight: 800; font-size: 1.35rem; line-height: .96; color: var(--ink) !important; }
.air-ribbon-wrap { position: relative; z-index: 2; margin-top: 14px; border-radius: 18px; padding: 10px 12px; background: #FFFFFF; border: 1px solid var(--line); box-shadow: 0 8px 20px rgba(15,23,42,.055); }
...
.air-ribbon-legend { display: flex; flex-wrap: wrap; gap: 8px 13px; margin-top: 9px; color: var(--slate) !important; font-size: .72rem; font-weight: 800; }
...
def page_critical(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with c2:
        kpi_card("Critical valid", fmt_int(len(crit_df)), note="observasi dengan critical", accent="#0284C7")
    with c3:
...
def page_data_quality(df: pd.DataFrame, full_df: pd.DataFrame, log_df: pd.DataFrame, audit_df: pd.DataFrame, validation_df: pd.DataFrame) -> None:
    ...
    with c2:
        kpi_card("Periode data", f"{start}<br>{end}", note="bukan real-time", accent="#0284C7")
    with c3:
