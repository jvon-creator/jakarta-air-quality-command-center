...
APP_SUBTITLE = (
    "Membaca napas kota melalui data ISPU: udara terkini, sinyal risiko, "
...
:root {
    ...
    --slate: #496677;
    --muted: #6C8798;
    --line: rgba(28, 86, 118, 0.14);
    --line-strong: rgba(28, 86, 118, 0.22);
    --oxygen: #12B981;
    --sky: #1BA7D8;
    --haze: #F2B705;
    --dust: #F97316;
...
.stApp {
    color: var(--ink);
    background:
        radial-gradient(circle at 12% -10%, rgba(27,167,216,.18), transparent 34%),
        radial-gradient(circle at 88% 4%, rgba(18,185,129,.13), transparent 30%),
        linear-gradient(180deg, #F5FBFF 0%, #ECF8FF 38%, #F7FAFC 100%);
    overflow-x: hidden;
}

/* Signature visual: lapisan udara tipis + partikel halus, cukup rendah agar konten tidak pernah tertutup. */
.stApp::before {
    content: "";
    position: fixed;
    inset: 0;
    ...
    pointer-events: none;
    background-image:
        radial-gradient(circle at 20% 22%, rgba(21, 50, 67, .055) 0 1px, transparent 1.6px),
        radial-gradient(circle at 80% 48%, rgba(242, 183, 5, .075) 0 1px, transparent 1.8px),
        linear-gradient(112deg, transparent 0%, rgba(27,167,216,.070) 27%, transparent 46%, rgba(18,185,129,.055) 72%, transparent 100%);
...
.stApp::after {
    ...
    pointer-events: none;
    opacity: .38;
    background: repeating-linear-gradient(168deg, transparent 0 22px, rgba(27,167,216,.10) 23px 24px, transparent 25px 56px);
...
[data-testid="stSidebar"] {
    background:
        linear-gradient(180deg, rgba(255,255,255,.96), rgba(237,248,255,.96)),
        radial-gradient(circle at top left, rgba(27,167,216,.18), transparent 42%);
...
.command-hero {
    ...
    background:
        radial-gradient(circle at 12% 14%, rgba(27,167,216,.18), transparent 33%),
        radial-gradient(circle at 92% 8%, rgba(18,185,129,.15), transparent 31%),
        linear-gradient(135deg, rgba(255,255,255,.97), rgba(235,248,255,.93));
...
.command-hero::before {
    ...
    bottom: -34px;
    height: 118px;
    background: linear-gradient(90deg, rgba(18,185,129,.26) 0%, rgba(27,167,216,.22) 28%, rgba(242,183,5,.20) 52%, rgba(249,115,22,.16) 72%, rgba(239,68,68,.12) 88%, rgba(124,58,237,.14) 100%);
    filter: blur(25px);
    opacity: .90;
    pointer-events: none;
}
.command-hero::after {
    ...
    inset: 0;
    background:
        radial-gradient(circle at 78% 16%, rgba(21,50,67,.075) 0 1px, transparent 1.8px),
        radial-gradient(circle at 68% 58%, rgba(242,183,5,.10) 0 1px, transparent 2px),
        repeating-linear-gradient(12deg, transparent 0 18px, rgba(27,167,216,.06) 19px 20px, transparent 21px 44px);
...
.hero-title {
    font-family: 'Barlow Condensed', 'Inter', sans-serif;
    font-size: clamp(2.55rem, 4.9vw, 5.75rem);
    line-height: .86;
...
.kpi-card::before {
    ...
    height: 136px;
    border-radius: 999px;
    background: color-mix(in srgb, var(--accent, #12B981) 17%, transparent);
    filter: blur(13px);
    opacity: .95;
}
.kpi-card::after {
    ...
    background: linear-gradient(90deg, var(--accent, #12B981), rgba(27,167,216,.16));
}
.kpi-label {
    color: var(--muted);
    font-family: 'JetBrains Mono', ui-monospace, monospace;
...
.warning-panel p { margin: 0; color: #5A4100; line-height: 1.58; }
...
def kpi_card(label: str, value: str, delta: str = "", note: str = "", accent: str = "#12B981") -> None:
...
def apply_fig_style(fig: go.Figure, height: Optional[int] = None, legend: bool = True) -> go.Figure:
    fig.update_layout(
        ...
        font=dict(family="Inter, Arial, sans-serif", color="#153243", size=12),
        title_font=dict(family="Barlow Condensed, Inter, sans-serif", color="#153243", size=20),
        margin=dict(t=52, r=24, b=42, l=32),
        hoverlabel=dict(bgcolor="#FFFFFF", bordercolor="rgba(27,167,216,.28)", font_color="#153243"),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
    ...
    fig.update_xaxes(
        ...
        tickfont=dict(color="#496677"),
        title_font=dict(color="#496677"),
    )
    fig.update_yaxes(
...
def add_threshold_lines(fig: go.Figure, include_all: bool = False) -> go.Figure:
    thresholds = [(101, "Ambang Tidak Sehat 101", "#F97316")]
    if include_all:
        thresholds.extend([(201, "Sangat Tidak Sehat 201", "#EF4444"), (301, "Berbahaya 301", "#7C3AED")])
...
def hero(current_page: str, df: pd.DataFrame, filtered: pd.DataFrame) -> None:
    if df.empty:
        ...
        validation_hint = "Clean dataset final"
        status = validation_hint

    filtered_hint = f"{fmt_int(len(filtered))} observasi aktif" if not filtered.empty else "Tidak ada observasi aktif"
...
def empty_state() -> None:
    st.error(
        "Tidak ada data setelah filter diterapkan. Longgarkan filter periode, stasiun, kategori, atau pencemar kritis."
    )
...
def fig_critical_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    fig = px.imshow(
        pivot,
        aspect="auto",
        color_continuous_scale=["#F5FBFF", "#A7E5F6", "#F2B705", "#EF4444"],
...
def fig_seasonal_heatmap(df: pd.DataFrame, metric: str) -> go.Figure:
    if metric == "Rata-rata ISPU":
        ...
        scale = ["#F5FBFF", "#A7E5F6", "#F2B705", "#F97316", "#EF4444"]
    else:
        ...
        scale = ["#F5FBFF", "#CDEFE7", "#F2B705", "#F97316", "#EF4444"]
...
def page_overview(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with c1:
        kpi_card("Rata-rata ISPU", fmt_float(avg_ispu, 1), note=ispu_status_from_average(avg_ispu), accent=CATEGORY_COLORS.get(ispu_status_from_average(avg_ispu), "#12B981"))
    with c2:
        kpi_card("Median ISPU", fmt_float(median_ispu, 1), note="kondisi tipikal", accent="#1BA7D8")
    with c3:
        kpi_card("Tidak Sehat+", fmt_pct(unhealthy, 1), note="frekuensi risiko", accent="#F97316")
    with c4:
...
def page_trend(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with k1:
        kpi_card("Tahun terakhir", str(max_year), note="dalam data terfilter", accent="#1BA7D8")
    with k2:
        kpi_card("Rata-rata ISPU", fmt_float(cur_avg, 1), delta=(f"vs {max_year-1}: {fmt_float(delta_avg,1)}" if not pd.isna(delta_avg) else ""), accent=CATEGORY_COLORS.get(ispu_status_from_average(cur_avg), "#12B981"))
    with k3:
        kpi_card("Tidak Sehat+", fmt_pct(cur_unhealthy, 1), delta=(f"vs {max_year-1}: {fmt_pct(delta_unhealthy,1)}" if not pd.isna(delta_unhealthy) else ""), accent="#F97316")
    with k4:
        worst_year_df = (
            df.groupby("tahun")
...
def page_station(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with c3:
        kpi_card("Stasiun risiko terendah", station_code(bottom["stasiun"]), note=bottom["stasiun"], accent="#12B981")
    with c4:
        kpi_card("Kesenjangan risiko", fmt_pct(gap, 1), note="selisih tertinggi vs terendah", accent="#F2B705")
...
def page_critical(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi_card("Pencemar dominan", top_crit, note=f"{fmt_pct(top_crit_pct, 1)} dari data critical", accent=CRITICAL_COLORS.get(top_crit, "#12B981"))
    with c2:
        kpi_card("Critical valid", fmt_int(len(crit_df)), note="observasi dengan critical", accent="#1BA7D8")
    with c3:
    ...
    with c4:
        kpi_card("Multi-critical", fmt_pct(multi_share, 1), note="kasus pencemar bernilai maksimum sama", accent="#F2B705")
...
def page_seasonal(df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with c3:
        kpi_card("Bulan relatif terbaik", str(best_avg["nama_bulan"]), note=f"Rata-rata {fmt_float(best_avg['rata_rata_ispu'],1)}", accent="#12B981")
    with c4:
        kpi_card("Pencemar bulan rawan", str(worst_risk["pencemar_dominan"]), note=f"pada {worst_risk['nama_bulan']}", accent=CRITICAL_COLORS.get(str(worst_risk["pencemar_dominan"]), "#F2B705"))
...
def page_data_quality(df: pd.DataFrame, full_df: pd.DataFrame, log_df: pd.DataFrame, audit_df: pd.DataFrame, validation_df: pd.DataFrame) -> None:
    ...
    status_accent = "#12B981" if status == "Layak digunakan" else "#F2B705"
    ...
    with c2:
        kpi_card("Periode data", f"{start}<br>{end}", note="bukan real-time", accent="#1BA7D8")
    with c3:
        kpi_card("Observasi final", fmt_int(total_final), note=f"{fmt_int(total_filtered)} aktif setelah filter", accent="#12B981")
    with c4:
        kpi_card("Stasiun", fmt_int(total_stations), note="SPKU dalam dataset", accent="#F2B705")
    with c5:
        kpi_card("Duplikasi final", fmt_int(final_dup), note=f"audit duplikasi: {fmt_int(total_audit)} baris", accent="#EF4444" if final_dup else "#12B981")
    ...
    with col1:
        ...
        fig.update_traces(marker_color="#1BA7D8", text=yearly["observasi"], textposition="outside", cliponaxis=False)
    ...
    with col2:
        ...
        fig.update_traces(marker_color="#12B981", text="observasi", textposition="outside", cliponaxis=False)
        st.plotly_chart(apply_fig_style(fig, height=380, legend=False), use_container_width=True)
    ...
    fig.update_traces(marker_color="#F2B705", texttemplate="%{text:.1f}%", textposition="outside", cliponaxis=False)
...
def main() -> None:
    ...
    st.markdown("---")
    st.caption(
        "Dashboard BI ISPU DKI Jakarta · Dibangun untuk mendukung keputusan berbasis data: kondisi terkini, tren, lokasi prioritas, pencemar kritis, pola musiman, dan kepercayaan data."
    )
